...
export default function ViewGoalsPage() {
  ...
  const getFilteredGoals = () => {
    ...
    if (searchText) {
      filtered = filtered.filter(goal =>
        ...
        goal.assignedTo.name.toLowerCase().includes(searchText.toLowerCase())
      );
    }
    ...
    switch (activeTab) {
      case 'my-goals':
        filtered = filtered.filter(goal => goal.assignedTo.id === user?.id);
        break;
      case 'created-by-me':
  ...
  const columns = [
      ...
      render: (record: Goal) => (
        <div>
          <Progress 
            ...
            strokeColor={{
              '0%': record.status === GoalStatus.OVERDUE ? '#ff4d4f' : '#108ee9',
              '100%': record.status === GoalStatus.OVERDUE ? '#ff7875' : '#87d068',
            }}
            size="small"
          />
    ...
    },
    {
      ...
      render: (record: Goal) => (
        <div>
          <div className="font-medium">{record.assignedTo.name}</div>
          <div className="text-gray-500 text-sm">{record.assignedTo.department}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (record: Goal) => (
        <Space>
          ...
          {(record.assignedTo.id === user?.id || record.createdBy.id === user?.id || hasRole(UserRole.ADMIN)) && (
    ...
    <div className="space-y-6">
      ...
      <Modal
        ...
        footer={[
          ...
          selectedGoal && (selectedGoal.assignedTo.id === user?.id || selectedGoal.createdBy.id === user?.id || hasRole(UserRole.ADMIN)) && (
          ...
          <div>
            ...
            <Descriptions bordered column={2}>
              ...
              <Descriptions.Item label="Assigned To">
                {selectedGoal.assignedTo.name} ({selectedGoal.assignedTo.email})
              </Descriptions.Item>
